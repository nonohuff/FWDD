...
def func_to_fit(times, noise_profile, *args, delta=False, **kwargs):
    ...
    if delta:
        ...
        C_t = coherence_decay_profile_delta(np.array(times), noise_spectrum_values)
    else:
        num_cpus = multiprocessing.cpu_count()
        if int(num_cpus) > 1:
            C_t, _, _, _ = parallel_coherence_decay(times, n, tau_p, integration_method, noise_profile, *args, initial_n_jobs=int(num_cpus), max_memory_per_worker=10**4, **kwargs)
        else:
            C_t, _, _, _ = zip(*[coherence_decay_profile_finite_peaks_with_widths(t, n, tau_p, integration_method, noise_profile, *args, **kwargs) for t in times])
...
# Modified loss function with scipy.special.huber
def loss_function(params, C_t_observed, times, noise_profile, param_structure, 
                  fixed_kwargs, delta=False, loss_type='huber', noise_level=None):
    """
