...
gaussian_noise = str_to_bool(sys.argv[3])
...
print(f"Add Gaussian noise?: {gaussian_noise}")
if gaussian_noise:
    print(f"Adding Gaussian noise with mu={mu} and sigma={sigma} to the data {copies} times.")
print("##################")
...
if assume_delta == True:
    ...
    noise_types = ["noise_spectrum_1f"]

# general way to convert h5 file stuff to same form that is stored in the .pkl file
# with pd.HDFStore(output_file, 'r') as store:
#     # for key, value in store.items():
#     #     print(store[key])
#     print(store.keys())
#     print(np.all(store["data"] == file_data["data"]))
#     # Get all columns except omega_range as regular key-value pairs
#     result = store["filter_function_args"].iloc[0].drop('omega_range').astype(int).to_dict()
#     # Add omega_range as a tuple of both values
#     result['omega_range'] = tuple(store["filter_function_args"]['omega_range'].values)
#     print(np.all(result == file_data["filter_function_args"]))
#     print(np.all(store["noise_profile_args"].values==file_data["noise_profile_args"]))
#     print(store["noise_profile"][0]==file_data["noise_profile"])
#     print()

# Initialize training and validation arrays as None
XTraining = None
YTraining = None
XValidation = None
YValidation = None
...
om = np.logspace(np.log10(finite_width_params["omega_range"][0]),
                 np.log10(finite_width_params["omega_range"][1]),
...
for name in noise_types:
    ...
    if XTraining is None:
        ...
        YTraining = data_Y_train
        XValidation = data_X_val
        YValidation = data_Y_val
    else:
        XTraining = np.concatenate((XTraining, data_X_train), axis=0)
...
# Additional information about model size
model_size = YTraining.shape[1]
...
except:  # Use the Default Strategy
  print("MirroredStrategy not available. Using Default Strategy.")
...
with strategy.scope():
    ...
    # Calculate optimal batch size based on number of GPUs
    # A good starting point is to multiply your single-GPU batch size by the number of GPUs
    base_batch_size = 32
    ...
    # run the model
    history = model.fit(XTraining,YTraining,batch_size=batch_size,epochs=200,validation_data=(XValidation,YValidation),callbacks=[early_stopping])
...
if gaussian_noise:
    ...
    plt.savefig(global_filepath + 'training_history_model_n_'+str(finite_width_params["N"])+"_tau_p_"+str(finite_width_params["tau_p"])+'.png')  # Save the figure
    # plt.show()
    print("Training history figure saved to " + global_filepath + 'training_history_model_n_'+str(finite_width_params["N"])+"_tau_p_"+str(finite_width_params["tau_p"])+'.png')