...
def build_dense(input_shape, output_shape, k_layers=6, r_units=100, negative_slope=0.1, activation_output='linear'):
    """
    Build a fully connected neural network with k hidden layers, each containing r units.
    ...
    negative_slope : float
        Slope for negative values in LeakyReLU activation
...
if assume_delta == True:
    print("Assuming Delta functions for Filter function pulses.")
    ...
    print("Loading first dataset: ", noise_types[0])
    X,Y = joblib.load(f"{global_filepath}training_data_{noise_types[0]}_{params}_finite_width.pkl")["data"]
...
# Add Gaussian noise
if gaussian_noise:
    X = add_gaussian_noise(X, mu, sigma)
...
model = build_dense((len(t_points), 1), (model_size,), k_layers=1000, r_units=2000, negative_slope=0.1, activation_output='linear')
...
model.save(global_filepath + 'trained_model_n'+str(finite_width_params["N"])+"_tau_p_"+str(finite_width_params["tau_p"])+'reduction_'+str(reduce)+"_model_dense"+'.keras')
...
plt.yscale('log')
plt.savefig(global_filepath + 'training_history_model_n'+str(finite_width_params["N"])+"_tau_p_"+str(finite_width_params["tau_p"])+'_reduction_'+str(reduce)+'_model_dense.png')  # Save the figure
# plt.show()
print("Training history figure saved to " + global_filepath + 'training_history_model_reduction_' + str(reduce) + '.png')