...
def noise_spectrum_combo(omega,f_params,lor_params,white_params):
...
def make_and_save_training_data(t_points, save_every_n, num_samples, noise_profile, *args, **kwargs):
    """
    Generate and save training data, splitting parameters into groups for intermittent saving.
    
    Updated for SLURM job arrays to include data from both global and task-specific files.
    ...
    **kwargs : dict
    ...
    total_tasks = int(os.getenv("SLURM_ARRAY_TASK_COUNT", 1))  # Provided by SLURM
    ...
    print(f"Task {task_id}: Checking existing data in {hdf_filename}")
    # print(f"Task {task_id}: Checking existing data in {base_save_name} and {task_save_name}")
        ...
        try:
            with pd.HDFStore(output_path, mode='r') as store:
                if 'noise_profile_args' in store:
                    for params in store['noise_profile_args'].values:
                        existing_params.append(tuple(params))
            print(f"Task {1}: Found {len(existing_params)} existing parameter sets.")
        except (FileNotFoundError, KeyError):
            print(f"Task {1}: No existing data file found.")
    ...
    base_samples_per_task = len(all_parameters) // total_tasks
    remainder = len(all_parameters) % total_tasks

    # Calculate start and end indices with remainder distribution
    if task_id < remainder:
        # First 'remainder' tasks get one extra parameter
        start_idx = task_id * (base_samples_per_task + 1)
        end_idx = start_idx + base_samples_per_task + 1
    else:
        # Remaining tasks get the base number of parameters
        start_idx = (task_id * base_samples_per_task) + remainder
        end_idx = start_idx + base_samples_per_task

    print(f"Task {task_id}: Processing parameters {start_idx} to {end_idx-1}.")
    task_parameters = all_parameters[start_idx:end_idx]

    # Filter out parameters that have already been computed
    new_parameters = [param for param in task_parameters if param not in existing_params]
    ...
    print(f"Task {task_id}: Generating data for {len(new_parameters)} new parameters.")
    ...
    for batch_idx in range(num_batches):
        ...
        print(f"Task {task_id}: Finished and saved batch {batch_idx + 1}/{num_batches}.")

    print(f"Task {task_id}: Completed all batches.")
...
A_values = np.linspace(0, 1, ceil(np.sqrt(num_samples)),dtype=np_data_type)  # Adjust the number of points as needed
...
num_1f = int(sys.argv[3])
# Get all num_1f-length combinations for both parameter arrays
A_combos = list(combinations(A_values, num_1f))
...
num_C = int(sys.argv[5])
C_combos = list(combinations(np.linspace(0, 5, 1000), num_C))
...
print(f"Number of 1/f noise parameters: {finite_width_params['N1f']}")
print(f"Number of Lorentzian noise parameters: {finite_width_params['Nlor']}")
print(f"Number of white noise parameters: {finite_width_params['NC']}")
print("##################")
...
# save_every_n = np.max((int(num_samples_lor/100),1)) # The number of parameter sets to process in each batch. For intermittent saving of results.
save_every_n = 10
num_samples = 2*10**4
print("Generating Training Data: Combo Noise Noise")
print(finite_width_params)
print()
print(combo_args)
print("****************************")
make_and_save_training_data(t_points,save_every_n,num_samples,noise_spectrum_combo,*combo_args,**finite_width_params)
